...
struct thread_data{
     ...
     */
    pthread_mutex_t *mutex;        // mutex to be locked/unlocked by the thread
    int wait_to_obtain_ms;         // time to wait before attempting to obtain the mutex
    int wait_to_release_ms;        // time to wait before releasing the mutex
    /**
